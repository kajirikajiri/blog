...
<Hint text="EffectsとFillはFigma右側メニューの「Design」に設定項目があります。Design, Prototype, Inspectの「Design」です。"/>
...
<Hint text="border-topがやりたいときは、Yの値を1にしましょう。" />
...
<Hint text="Material-UIのTextFieldの場合にshadowを使用しない理由が分かっていません。推測では太さが2px以上の場合、shadowでは再現できなかった？もしくはdefaultのLineとactiveのLineで色を分けて表示する必要があった？"/>
...
<MyLink link="/design-learn-with-figma-1" text="エンジニアがfigmaでデザインを学習する"/>