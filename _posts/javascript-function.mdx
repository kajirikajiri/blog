...
いつも通りのfunctionに見えます。`returnNumberFunction`を**呼び出すときは()を後ろにつける**と思います。これをfunctionを定義した時に実行できます。なので
...
このような書き方が可能になります。また、()は結構なんでもできます。一応こんな書き方もできるんだと思ってください。
...
<MyLink link="/javascript-promise" text="JavaScriptのpromiseの使い方"/>