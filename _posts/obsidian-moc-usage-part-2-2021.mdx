...
<NotebookList head="概要" items={['MOCとMOCをリンクでつなぎます', 'MOCの中でタグを使い、MOCの中で分類します', '自分の場合。 Home MOC > Interests MOC > Programming MOC']} />

<MyLink link="/obsidian-usage-2021#__h2_10__" text="リンクの使い方はこちら"/>
...
<NotebookList head="小まとめ" items={['Interests MOCはHome MOCをいくつかまとめたグループのようになっている']} />
...
<NotebookList head="小まとめ" items={['lyt-kitではMOCを何層にもつなげている']} />
...
<NotebookList head="小まとめ" items={['MOCの最初の行には親になるMOCのリンクを書く']} />
...
<NotebookList head="tag:#moneyで検索してみつかる" items={['#money', '#money/account', '#money/virtualCurrency', '#money/virtualCurrency/bitcoin']} />

しかし、`tag:#mone`と検索しても`#money`はみつかりません。

<NotebookList head="みつからない" items={['#mone']} />
...
<NotebookList head="使い方" items={['Daily notes(ObsidianのCore plugins)のショートカットキー起動でファイル名にTimestampがついたファイルが生成されます。(ディレクトリは Brain > Timestamps > DailyNotes　にファイルが作成されます。)', 'YYYYMMDDHHmmssが最初から入力されてるんで、後ろにjavascript-debugを入力してファイル名を確定します。', 'ファイルの入力状態になったら、一番上にMOCへのリンクを入力します(今回はProgrammingです)', '一番下にはタグを書きます。(programming/javascript, debugのタグを追加します)', "後は真ん中にconsole.log('kajiri')とか書いておきます。"]} type="number" />
...
<MyLink link="/obsidian-usage-2021" text="Obsidianの使い方"/>
<MyLink link="/obsidian-moc-usage-2021" text="Obsidian MOCの使い方 第１話"/>
<MyLink link="/obsidian-vscode-extension" text="Obsidian vscode拡張機能について公式サイトで調べた"/>
<MyLink link="/obsidian-ios-android-mobile-app" text="Obsidianのiosとandroidのモバイルアプリが公開された"/>
