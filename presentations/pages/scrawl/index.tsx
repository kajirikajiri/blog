...
export const Scrawl = (props: Props) => {
  return (
            ...
            return (
              <div style={{ margin: 16 }} key={s.id}>
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{ __html: marked(s.body) }}
                />
              </div>
            );
          })}
...
const getLastScrawlCursor = async () => {
  ...
  const res = await getLastOneScrawl();
  return res.data.repository.discussions.edges[0].cursor;
};
...
export const getStaticProps = async (): GetStaticPropsResult => {
  const getAllScrawls = async () => {
    ...
    while (InfiniteLoop) {
      ...
      const { data } = await getScrawls(cursor);
      const { edges } = data.repository.discussions;
